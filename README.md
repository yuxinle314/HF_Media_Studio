...
一个面向 **Windows 10/11**（以及任何现代浏览器）的 Web 应用 / PWA，采用 **Apple HIG 风格** 界面。当前 `HF_Media_Studio_Arm` 分支为精简版本，提供三类功能：

1. **摄像头拍照 + 压缩** —— 调用电脑摄像头拍照，自动压缩到 **50kB / 100kB / 200kB / 256kB**，分辨率可在 `480×270 / 960×540 / 1024×768 / 1920×1080` 间选择或自动匹配，并以 256kB（256000 Bytes）作为传输硬上限。
2. **图片文件压缩** —— 选择 / 拖入任意图片，或在手机上拍照导入，按相同的目标大小与分辨率压缩；局域网模式下可把压缩结果上传保存到服务电脑。
3. **语音录音 + 声码器压缩** —— **按住按钮录音、松开结束**（最长 60s，带充能进度条，60s = 100%），或选择音频文件，用 **Codec2 超低码率声码器** 编码 / 转码（700C ≈ 700 bps，60s 仅约 5KB），并可把已有 `.c2` 文件转成普通播放器可播放的 MP3。

> 语音转文字和视频压缩转码功能已从该分支移除；照片与音频压缩 / 转码在浏览器本地处理。
