...
2. **摄像头拍照 + 压缩** —— 调用电脑摄像头拍照，自动压缩到 **50kB / 100kB / 200kB / 256kB**，分辨率可在 `480×270 / 960×540 / 1024×768 / 1920×1080` 间选择或自动匹配，并以 256kB（256000 Bytes）作为传输硬上限；压缩结果可点开大图预览，也可保存为压缩文件。
3. **图片文件压缩** —— 选择 / 拖入任意图片，或在手机上拍照导入，按相同的目标大小与分辨率压缩；局域网模式下可把压缩结果上传保存到服务电脑，也可保存为压缩文件。
4. **视频转码压缩** —— 选择本地视频后上传到服务电脑，由服务端 `ffmpeg.exe` 以 H.265/HEVC 优先、缺少时回退 H.264/AVC 的方式生成 MP4，截取最长 20 秒，可选 640×480 / 320×240，1~5 FPS、去除音频，并以 256kB（256000 Bytes）作为传输硬上限；手机访问时可在压缩完成后保存 / 分享回手机本地。
...
局域网模式下，手机或其它设备压缩图片后可点击「上传到服务电脑」。一键服务会把成品保存到运行服务电脑的 `server_uploads/pics/` 目录；该目录为本地运行产物，不会提交到 Git。iPhone 上不要把压缩结果保存到「照片」相册作为发送源，因为系统相册可能重新编码导致文件变大；应使用「保存压缩文件」并在系统提示中选择下载 / 存储到「文件」。

视频转码也在服务电脑完成，输出保存到 `server_uploads/videos/`。手机通过局域网访问页面并上传视频后，压缩结果页会提供「保存/分享至手机」「下载到本机」「复制链接」三个入口；移动浏览器支持文件分享时可直接调出系统保存 / 分享面板，不支持时会回退为打开或下载压缩视频。服务电脑需要能找到 `ffmpeg.exe`：可以放在 `tools/ffmpeg.exe`，或把解压后的 ffmpeg 文件夹放到 `tools/`，或把 ffmpeg 加入系统 PATH，或设置环境变量 `FFMPEG_PATH` 指向 ffmpeg.exe。`start.bat` 会在启动时自动检测 ffmpeg，缺少时会提示，但不影响其它功能启动。
...
- 压缩结果预览和大图使用压缩后 Blob 转出的 `data:` 图片数据；大图仅用于查看，精确发送文件应使用「保存压缩文件」或上传接口。
- 一键服务提供 `POST /api/upload`，用于把局域网设备上的压缩结果写入服务电脑 `server_uploads/pics/`，并返回附件下载链接；`GET /api/file/download?subdir=pics&file=<filename>` 用于把压缩后图片按原始字节回传给手机或电脑保存。
...
- 转码接口会返回预览链接和附件下载链接；`GET /api/video/download?file=<filename>` 用于把压缩后 MP4 回传给手机或电脑保存。
