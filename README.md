...
2. **摄像头拍照 + 压缩** —— 调用电脑摄像头拍照，自动压缩到 **50kB / 100kB / 200kB / 256kB**，分辨率可在 `480×270 / 960×540 / 1024×768 / 1920×1080` 间选择或自动匹配，并以 256kB（256000 Bytes）作为传输硬上限；压缩结果可点开大图，便于手机长按保存。
3. **图片文件压缩** —— 选择 / 拖入任意图片，或在手机上拍照导入，按相同的目标大小与分辨率压缩；局域网模式下可把压缩结果上传保存到服务电脑，也可点开大图后在手机上长按保存。
